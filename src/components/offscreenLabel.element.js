...
import { OffscreenLabelStyles } from "./styles.store.js";
...
export class OffscreenLabel extends HTMLElement {
  ...
  set update({ boundingRect, isFixed }) {
    this.style.setProperty(
      "--top",
      `${boundingRect.y + (isFixed ? 0 : window.scrollY)}px`
    );
    ...
    this.style.setProperty(
      ...
      `${
        boundingRect.width +
        (window.innerWidth - boundingRect.x - boundingRect.width - 20)
      }px`
    );
  ...
  render(node_label_id) {
    this.$shadow.host.setAttribute(
      "data-label-id",
      node_label_id || "label_" + Number(new Date())
    );
...
export function createOffscreenLabelIndicator(
  node_label_id,
  text,
  hoverText,
  left,
  top,
  color,
  adjustRightSideToCount
) {
  const existing = document.querySelectorAll(
    `visbug-offscreen-label[id=${text}]`
  );
  ...
  if (existing.length) {
    ...
    instance.style.setProperty(
      "--hover-text",
      `"\\00a0 ${hoverText ? "offscreen label: " + hoverText : instance.count}"`
    );
    if (adjustRightSideToCount) {
      left = left.includes("calc(")
        ? left.replace(")", ` - ${instance.count.toString().length}ch)`)
        : `${instance.count.toString().length}ch`;
      instance.style.setProperty("--left", left);
    }
  ...
  label.style.setProperty(
    "--hover-text",
    `"\\00a0 ${hoverText ? "offscreen label: " + hoverText : label.count}"`
  );
...
export function removeOffscreenLabelIndicators() {
  document.querySelectorAll("visbug-offscreen-label").forEach((e) => {
    e.seen = {};
    e.count = 0;
    e.text = "";
