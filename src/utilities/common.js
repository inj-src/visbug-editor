...
export const getNodeIndex = (el) => {
  return [...el.parentElement.parentElement.children].indexOf(el.parentElement);
};
...
export const showHideSelected = (el, duration = 750) => {
  ...
  if (timeoutMap[nodeKey(el)]) clearTimeout(timeoutMap[nodeKey(el)]);
...
export const showHideNodeLabel = (el, show = false) => {
  ...
  nodes.length && show
    ...
    : nodes.forEach((el) => (el.style.display = null));
};
...
export const isOffBounds = (node) =>
  ...
  (node.closest("vis-bug") ||
    ...
    node.closest("visbug-offscreen-label"));

export const isSelectorValid = ((qs) => (selector) => {
  try {
    qs(selector);
  } catch (e) {
    return false;
...
export const onRemove = (element, callback) => {
  const parent = element.parentNode ? element.parentNode : document.body;
